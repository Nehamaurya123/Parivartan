...
export const AddBooths: FC<any> = ({ onSave, onClose }) => {
  const [error, setError] = useState("");
  const [file, setFile] = useState<any>(null);

  const addAction = async (event: any) => {
    event.preventDefault();
    const form = new Validator("addbooths");

    if (!form.checkDirty()) form.validate();
    if (form.valid()) {
      var data = new FormData();
      data.append("file", file);
      let res: any = await API.post(APIS.BOOTH.SAVE_BOOTHS, data, {
    ...
    <div className="modal-news">
      ...
      >
        <div className="modal-header">
          <div className="title">
            {"Add"} Booths{" "}
            <a href="./other/BoothSample.xlsx">Download Sample</a>
          </div>
        </div>
        ...
        <div className="modal-footer modal-news-footer">
          ...
          {/* {(this.props.success!='')?<div className="successMessage">{this.props.success}</div>:''} */}
