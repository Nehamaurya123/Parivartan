...
const AddMpAssistant = ({ booths, record, onSave, onClose }: typeProps) => {
  const [error, setError] = useState("");
  const [data, setData] = useState(record ? record : {});
  console.log("record", record);
  const [options, setOptions] = useState([]);
  const [selectedValues, setSelectedValues] = useState<any>(record ? record.boothids.map((b:any, i: number)=>({label: record.boothnames[i] ,value: b})) : []);
  ...
  const addAction = async (event: any) => {
    ...
    const form = new Validator("addassistant");
    ...
    if (form.valid()) {
      ...
      selectedValues.map((a: any) => {
        formData.booths.push(a.value);
        formData.boothids.push(a.value);
        formData.boothnames.push(a.label);
      });
      let res: any = await API.post(APIS.ASSISTANT.SAVE_ASSISTANT, formData);
      console.log(res, "ASSIS-UPD");
